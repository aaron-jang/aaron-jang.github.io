...
<script type="application/ld+json">
{
  ...
  "mainEntity": [
    {
      "@type": "Question",
      "name": "n8n은 Zapier와 어떻게 다른가요?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "n8n은 오픈소스로 자체 서버에 설치(셀프호스팅)하거나 클라우드 버전을 사용할 수 있어 데이터 프라이버시와 비용 면에서 유리합니다. Zapier는 SaaS 서비스로 설정이 간단하지만 유료 플랜이 비싸고 데이터가 외부 서버에 저장됩니다. 기술적 이해가 있고 비용을 절감하고 싶다면 n8n이 적합합니다."
      }
    },
    {
      "@type": "Question",
      "name": "n8n을 무료로 사용할 수 있나요?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "n8n은 셀프호스팅 방식으로 완전 무료로 사용할 수 있습니다. 클라우드 버전은 월 20달러부터 시작하는 유료 플랜이 있습니다. 개인 및 소규모 팀은 셀프호스팅(무료)으로 시작하고, 서버 관리가 부담스럽다면 클라우드 버전이 편리합니다."
      }
    },
    {
      "@type": "Question",
      "name": "n8n으로 만들 수 있는 대표적인 자동화 워크플로우는?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Gmail 수신 → Slack 알림, 웹사이트 폼 제출 → 노션 데이터베이스 저장, 구글 스프레드시트 데이터 → 카카오톡 메시지 발송 등 다양한 자동화가 가능합니다. API가 있는 서비스라면 대부분 n8n으로 연동해 반복 작업을 자동화할 수 있습니다."
      }
    }
  ]
}
