---
# the default layout is 'page'
icon: fas fa-info-circle
order: 4
title: About
description: "수수라이프 - IT, 재테크, 생활정보 블로그"
---

## 수수라이프 소개

안녕하세요. **수수**입니다.

**수수라이프(soosoo.life)**는 IT/테크, 재테크, 생활 정보를 다루는 한국어 블로그입니다. 복잡한 정보를 누구나 이해할 수 있도록 쉽고 명확하게 정리하는 것을 목표로 합니다.

### 주요 콘텐츠

- **IT / 테크** : VPN 비교, API 가이드, 개발 도구 리뷰, 셀프호스팅 등
- **재테크 / 금융** : 투자 정보, 보조금 신청 가이드, 실적 분석 등
- **생활 정보** : 유용한 팁, 제품 리뷰, 실생활 가이드 등

### 운영 원칙

- 직접 경험하고 검증한 정보만 공유합니다
- 최신 정보를 유지하기 위해 글을 주기적으로 업데이트합니다
- 출처와 근거를 명확히 합니다

### 무료 도구

블로그와 함께 누구나 사용할 수 있는 무료 도구들도 제공합니다:

| 도구 | 설명 |
|------|------|
| [불렛저널 플래너](/bullet_journal_planner/) | 온라인 불렛저널 플래너 |
| [오늘 뭐 했지?](/todaydone/) | 하루 기록 도구 |
| [텐비 페낭 링크](/tenby-penang-links/) | 유용한 링크 모음 |

### 연락처

- Email: soosoolife.k@gmail.com
- Blog: [soosoo.life](https://soosoo.life)

---

**블로그가 도움이 되셨나요?** 커피 한 잔으로 응원해주시면 더 좋은 콘텐츠를 만드는 데 큰 힘이 됩니다 :)

<a href="https://ko-fi.com/soosoolab" target="_blank" rel="noopener">
  <img class="kofi-badge-light" src="/assets/images/support_me_on_kofi_badge_beige.png" alt="Support me on Ko-fi" height="48">
  <img class="kofi-badge-dark" src="/assets/images/support_me_on_kofi_badge_dark.png" alt="Support me on Ko-fi" height="48">
</a>
